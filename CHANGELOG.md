...
- `Server: uvicorn` response header is now stripped before responses leave the app. An attacker who sees it can narrow the version range and match it against public CVE trackers; legitimate clients get nothing useful out of the field. Applied for all paths in the same middleware that sets the rest of the security headers.
